/*
 * This file is generated by jOOQ.
 */
package org.oana.storage.tables.pojos;


import jakarta.persistence.Column;
import jakarta.persistence.Entity;
import jakarta.persistence.GeneratedValue;
import jakarta.persistence.GenerationType;
import jakarta.persistence.Id;
import jakarta.persistence.Table;

import java.time.OffsetDateTime;

import org.oana.storage.tables.interfaces.IExerciseDifficulties;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes", "this-escape" })
@Entity
@Table(
    name = "exercise_difficulties",
    schema = "public"
)
public class ExerciseDifficulties implements IExerciseDifficulties {

    private static final long serialVersionUID = 1L;

    private Long id;
    private OffsetDateTime createdAt;
    private String name;

    public ExerciseDifficulties() {}

    public ExerciseDifficulties(IExerciseDifficulties value) {
        this.id = value.getId();
        this.createdAt = value.getCreatedAt();
        this.name = value.getName();
    }

    public ExerciseDifficulties(
        Long id,
        OffsetDateTime createdAt,
        String name
    ) {
        this.id = id;
        this.createdAt = createdAt;
        this.name = name;
    }

    /**
     * Getter for <code>public.exercise_difficulties.id</code>.
     */
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "id", nullable = false)
    @Override
    public Long getId() {
        return this.id;
    }

    /**
     * Setter for <code>public.exercise_difficulties.id</code>.
     */
    @Override
    public void setId(Long id) {
        this.id = id;
    }

    /**
     * Getter for <code>public.exercise_difficulties.created_at</code>.
     */
    @Column(name = "created_at", nullable = false, precision = 6)
    @Override
    public OffsetDateTime getCreatedAt() {
        return this.createdAt;
    }

    /**
     * Setter for <code>public.exercise_difficulties.created_at</code>.
     */
    @Override
    public void setCreatedAt(OffsetDateTime createdAt) {
        this.createdAt = createdAt;
    }

    /**
     * Getter for <code>public.exercise_difficulties.name</code>.
     */
    @Column(name = "name")
    @Override
    public String getName() {
        return this.name;
    }

    /**
     * Setter for <code>public.exercise_difficulties.name</code>.
     */
    @Override
    public void setName(String name) {
        this.name = name;
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (obj == null)
            return false;
        if (getClass() != obj.getClass())
            return false;
        final ExerciseDifficulties other = (ExerciseDifficulties) obj;
        if (this.id == null) {
            if (other.id != null)
                return false;
        }
        else if (!this.id.equals(other.id))
            return false;
        if (this.createdAt == null) {
            if (other.createdAt != null)
                return false;
        }
        else if (!this.createdAt.equals(other.createdAt))
            return false;
        if (this.name == null) {
            if (other.name != null)
                return false;
        }
        else if (!this.name.equals(other.name))
            return false;
        return true;
    }

    @Override
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = prime * result + ((this.id == null) ? 0 : this.id.hashCode());
        result = prime * result + ((this.createdAt == null) ? 0 : this.createdAt.hashCode());
        result = prime * result + ((this.name == null) ? 0 : this.name.hashCode());
        return result;
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder("ExerciseDifficulties (");

        sb.append(id);
        sb.append(", ").append(createdAt);
        sb.append(", ").append(name);

        sb.append(")");
        return sb.toString();
    }

    // -------------------------------------------------------------------------
    // FROM and INTO
    // -------------------------------------------------------------------------

    @Override
    public void from(IExerciseDifficulties from) {
        setId(from.getId());
        setCreatedAt(from.getCreatedAt());
        setName(from.getName());
    }

    @Override
    public <E extends IExerciseDifficulties> E into(E into) {
        into.from(this);
        return into;
    }
}
